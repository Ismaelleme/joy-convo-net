    ...
    <div className="relative w-full h-full snap-start snap-always flex-shrink-0">
      ...
      <div className="absolute right-3 bottom-32 z-10 flex flex-col items-center gap-5">
        ...
        <div className="relative">
          ...
          <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-5 h-5 bg-primary rounded-full flex items-center justify-center border border-background">
        ...
        <button onClick={toggleLike} className="flex flex-col items-center gap-1">
          <Heart className={`w-7 h-7 ${liked ? 'fill-red-500 text-red-500' : 'text-foreground'} transition-colors`} />
        ...
        <button onClick={() => setSaved(!saved)} className="flex flex-col items-center gap-1">
          <Bookmark className={`w-7 h-7 ${saved ? 'fill-amber-400 text-amber-400' : 'text-foreground'} transition-colors`} />
        </button>
      </div>

      {/* Bottom info */}
      <div className="absolute bottom-6 left-3 right-16 z-10">
        <p className="font-bold text-sm text-foreground mb-1">@{video.userName.toLowerCase().replace(' ', '_')}</p>
        <p className="text-sm text-foreground/90 leading-relaxed mb-2">{video.description}</p>
        ...
        {video.music && (
          <div className="flex items-center gap-2">
            <Music className="w-3 h-3 text-foreground animate-spin" style={{ animationDuration: '3s' }} />
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-center pt-4 pb-2">
        <h1 className="text-lg font-bold text-foreground drop-shadow-lg">Explorar</h1>
      </div>

      {/* Video feed with snap scroll */}
      <div
        ref={containerRef}
